...
- **Author**: `算法`
- **Cover image**: Deep blue tech background, two lines of white bold text "BMS 算法追踪" and date "YYYY-MM-DD" at **equal visual glyph height**, rendered via PIL (see pitfall 22). Size reference (matches 2026-08-02 issue): CJK font ~76pt / date ~96pt on a 1800x766 canvas → glyph height ≈ 10% of canvas height, text block ≈ 25% of height centered at ~49%; generous whitespace, title width ≈ 28% of canvas
...
22. **Cover must be rendered via PIL, not AI generation (CRITICAL)**: AI image generators CANNOT reliably produce two text lines at equal font size (CJK always renders bigger than digits at equal point size, or hierarchy gets re-imposed), and they embed watermarks plus background artifacts that land behind the date text. Render deterministically instead: draw the background programmatically (deep-blue vertical gradient `#08142E`→`#164296`, soft radial glow, thin circuit lines with node dots at bottom), then draw text on a 1800x766 canvas with `ImageFont.truetype(r"C:\Windows\Fonts\msyhbd.ttc", 76)` for the CJK line. To make 汉字 and date LOOK the same size, scale the date font by `cjk_glyph_height / digit_glyph_height` measured via `textbbox`, probing digits **at CJK_SIZE, never a hardcoded probe size** (~1.26x for msyhbd — same point size makes digits look ~25% smaller because CJK fills the em square while digits only reach cap height; a hardcoded 200pt probe silently breaks when CJK_SIZE changes). Target glyph height ≈ 10% of canvas height, block ≈ 25% of height centered at ~49% (see the 2026-08-02 issue cover for the reference look — do NOT use 200pt, it fills ~75% width and looks oversized). Center each line horizontally. Save as JPEG quality 92.
