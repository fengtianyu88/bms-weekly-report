...
- **Author**: `算法`
- **Cover image**: Deep blue tech background, two lines of white bold text "BMS 算法追踪" and date "YYYY-MM-DD" at **equal visual glyph height**, rendered via PIL (see pitfall 22). Size reference (approved by user 2026-08-18): CJK font ~110pt / date ~139pt on a 1800x766 canvas → glyph height ≈ 14% of canvas height, title width ≈ 40% of canvas, block centered at ~49%; generous whitespace
...
22. **Cover must be rendered via PIL, not AI generation (CRITICAL)**: AI image generators CANNOT reliably produce two text lines at equal font size (CJK always renders bigger than digits at equal point size, or hierarchy gets re-imposed), and they embed watermarks plus background artifacts that land behind the date text. Render deterministically instead: draw the background programmatically (deep-blue vertical gradient `#08142E`→`#164296`, soft radial glow, thin circuit lines with node dots at bottom), then draw text on a 1800x766 canvas with `ImageFont.truetype(r"C:\Windows\Fonts\msyhbd.ttc", 110)` for the CJK line. To make 汉字 and date LOOK the same size, scale the date font by `cjk_glyph_height / digit_glyph_height` measured via `textbbox`, probing digits **at CJK_SIZE, never a hardcoded probe size** (~1.27x for msyhbd — same point size makes digits look ~25% smaller because CJK fills the em square while digits only reach cap height; a hardcoded 200pt probe silently breaks when CJK_SIZE changes). Target glyph height ≈ 14% of canvas height, title width ≈ 40%, block centered at ~49% (110pt CJK + 139pt date is the approved look; 200pt fills ~75% width and looks oversized, 76pt was judged too small). Center each line horizontally. Save as JPEG quality 92.
