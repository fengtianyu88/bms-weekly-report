...
### 7.4 封面图生成（PIL 确定性渲染，2026-08-18 定稿）

**禁止用 AI 图像生成**：AI 无法可靠控制两行文字的字号关系（同点数时汉字恒大于数字，或强加层级），且自带水印与背景杂质。必须用 PIL 程序化渲染：

- 画布：`1800x766`（微信封面 2.35:1）
- 背景：程序绘制——深蓝垂直渐变 `#08142E`→`#164296` + 中央柔光 + 底部电路线条与节点
- 文字：微软雅黑 Bold（`C:\Windows\Fonts\msyhbd.ttc`）
- **最终定稿字号（层级排版，约 1.8:1）**：标题行「BMS 算法追踪」**160pt**，日期行「YYYY-MM-DD」**111pt**
- 目标比例：标题字高 ≈ 画面高 20%（宽 ≈ 59%），日期字高 ≈ 11%（宽 ≈ 36%），文字块垂直居中于画面 49% 线
- 演进记录（勿回退）：200pt 等高版过大（宽 75%）→ 76pt 等高版偏小 → 110/139pt 等高版被 160/111pt 层级版取代
- 保存 JPEG quality 92

参考实现：`examples/bms_report_20260817/make_cover.py`
...
| 封面图格式不符 | 未参考历史文章 | 用 PIL 程序化渲染（160/111pt 层级），见 7.4 节 |
